# Train a Generative Adversarial Network to generate MNIST Handwritten Digits.
# Modified from the blog https://blogs.rstudio.com/ai/posts/2018-08-26-eager-dcgan/
...
# Separate the training set into batches of size 256
buffer_size <- 60000
batch_size <- 256
...
# Define the discriminator structure
...
# Define the losses. Note that setting from_logits = TRUE allows y_pred to be in
# [-inf, inf], since this number will be considered as logit(p) with p in [0, 1].
...
train <- function(dataset, epochs, noise_dim) {
  for (epoch in seq_len(num_epochs)) {
    ...
    if(epoch %% 10 == 0){
      ...
      # Generate images to monitor the training process
